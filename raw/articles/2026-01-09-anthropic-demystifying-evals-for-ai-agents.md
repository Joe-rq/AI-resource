...
tags: [agent-evals, eval-methodology, grading, coding-agents, conversational-agents, research-agents, computer-use-agents, pass-at-k, agent-harness]
---

# Demystifying evals for AI agents

> Source: [Anthropic Engineering](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents) · Published Jan 09, 2026
>
> Compiled from web fetch (markdown conversion) on 2026-09-13. Page chrome (nav, footer, sitemap) stripped; image references preserved as Next.js URLs.
...
|---|---|---|
| String match checks (exact, regex, fuzzy, etc.) | Fast | Brittle to valid variations that don't match expected patterns exactly |
| Binary tests (fail-to-pass, pass-to-pass) | Cheap | Lacking in nuance |
| Static analysis (lint, type, security) | Objective | Limited for evaluating some more subjective tasks |
| Outcome verification | Reproducible | |
| Tool calls verification (tools used, parameters) | Easy to debug | |
| Transcript analysis (turns taken, token usage) | Verify specific conditions | |

**Model-based graders**

| Methods | Strengths | Weaknesses |
|---|---|---|
| Rubric-based scoring | Flexible | Non-deterministic |
| Natural language assertions | Scalable | More expensive than code |
| Pairwise comparison | Captures nuance | Requires calibration with human graders for accuracy |
| Reference-based evaluation | Handles open-ended tasks | |
| Multi-judge consensus | Handles freeform output | |

**Human graders**

| Methods | Strengths | Weaknesses |
|---|---|---|
| SME review | Gold standard quality | Expensive |
| Crowdsourced judgment | Matches expert user judgment | Slow |
| Spot-check sampling | Used to calibrate model-based graders | Often requires access to human experts at scale |
| A/B testing | | |
...
Test both the cases where a behavior *should* occur and where it *shouldn't*. One-sided evals create one-sided optimization. For instance, if you only test whether the agent searches when it should, you might end up with an agent that searches for almost everything. Try to avoid [class-imbalanced](https://developers.google.com/machine-learning/crash-course/overfitting/imbalanced-datasets) evals. We learned this firsthand when building evals for web search in [Claude.ai](http://claude.ai/redirect/website.v1.fc53513f-1faf-4277-b75c-ff4e550d4b33). The challenge was preventing the model from searching when it shouldn't, while preserving its ability to do extensive research when appropriate. The team built evals covering both directions: queries where the model should search (like finding the weather) and queries where it should answer from existing knowledge (like "who founded Apple?"). Striking the right balance between undertriggering (not searching when it should) or overtriggering (searching when it shouldn't) was difficult, and took many rounds of refinements to both the prompts and the eval. As more example problems come up, we continue to add to evals to improve our coverage.
...
Teams without evals get bogged down in reactive loops—fixing one failure, creating another, unable to distinguish real regressions from noise. Teams that invest early find the opposite: development accelerates as failures become test cases, test cases prevent regressions, and metrics replace guesswork. Evals give the whole team a clear hill to climb, turning "the agent feels worse" into something actionable. The value compounds, but only if you treat evals as core component, not an afterthought.
...
*Compiled from `https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents` via WebFetch (markdown conversion) on 2026-09-13. Page chrome (nav, footer, sitemap, social links) stripped; inline images referenced but not embedded.*