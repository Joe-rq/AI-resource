...
source: "url"
source_file: "https://platform.claude.com/docs/en/managed-agents/multi-agent"
created: "2026-05-18T02:49:27Z"
...
<Note>
All Managed Agents API requests require the `managed-agents-2026-04-01` beta header. The SDK sets this beta header automatically. </Note>
...
<Note>
A maximum of 25 concurrent threads are supported. The coordinator can call multiple copies of a single agent in the roster, creating multiple threads associated with one `agent`. </Note>

<Tabs>
  <Tab title="List threads"> List all threads associated with a session as follows: <CodeGroup>
...
The full list includes the primary thread. `parent_thread_id` will be null for the primary thread.   </Tab>

  <Tab title="Interrupt a session thread"> Send `user.interrupt` with `session_thread_id` to stop a specific thread. Omitting `session_thread_id` targets the primary thread.
...
Against a child thread blocked on `requires_action`, the interrupt marks each pending tool call denied and re-emits `session.thread_status_idle` with `stop_reason: end_turn` directly; the model is not sampled. Against a thread already at `idle`, the interrupt is a no-op.   </Tab>

  <Tab title="Archive a session thread"> Optionally archive a session thread when it has finished its work. This frees up a thread against the 25 thread limit.
...
<Tabs>
  <Tab title="Stream session thread events"> <CodeGroup>
...
````
  ...
  <Tab title="List session thread events"> List all past session thread events to pull a complete history.
