...
type: query
created: 2026-05-21
sources: [[Claude-Code-Subagent]], [[summaries/14-building-skill-for-claude]], [[summaries/09-claude-subagent-tutorial]]
...
- **Skill**：是一组打包的指令（文件夹形式），教 Claude 如何处理特定任务或工作流。它解决的问题是避免在每次对话中重复解释偏好、流程和领域专业知识 [[summaries/14-building-skill-for-claude]]。
...
- **Skill 适用场景**：生成遵循规范的前端设计、用一致方法论进行研究、创建遵循团队风格指南的文档、编排多步骤流程、MCP 集成增强 [[summaries/14-building-skill-for-claude]]。
...
4.  **定义方式**：Skill 是一个包含 `SKILL.md` 文件的文件夹。Subagent 是一个带 YAML 头部的 Markdown 文件，放在 `.claude/agents/` 目录下 [[Claude-Code-Subagent]] [[summaries/14-building-skill-for-claude]]。
